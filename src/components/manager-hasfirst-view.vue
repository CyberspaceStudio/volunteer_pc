      ...
      <a-col :span="23" :offset="0.5">
          ...
          <a-row>
            <a-col :span="4" :offset="6">
                <a-button type='default' size='large' @click="passThis">通过</a-button>
            </a-col>
...
import {get_first_interview_done, first_interview_PASS, first_interview_out} from '../utils/API'
...
export default {
    ...
    async mounted(){
        ...
        let info = await get_first_interview_done(this,1).catch((err)=>{
            console.log(err);
    ...
    methods:{
        async passThis(){
            ...
            await first_interview_PASS(this,info).catch((err)=>{
                console.log(err);
                 this.$notification.open({
                    message: '亲，出错啦！',
                    description: '请求失败，请稍候重试',
            ...
            info = await get_first_interview_done(this).catch((err)=>{
                console.log(err);
                 this.$notification.open({
                    message: '亲，出错啦！',
                    description: '请求失败，请稍候重试',
        ...
        async outThis(){
            ...
            await first_interview_out(this,info).catch((err)=>{
                console.log(err);
                 this.$notification.open({
                    message: '亲，出错啦！',
                    description: '请求失败，请稍候重试',
